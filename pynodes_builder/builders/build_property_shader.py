import bpy, itertools, mathutils, json, re, math
    ...
    for attr in dir(bnode):
        ...
        if enum_values is not None:
            ...
            if attr_type.startswith("bpy.types"):
                value = None
            if attr_type == "float" and len(str(value)) > 10:
                if attr.startswith("sun"):
                    value = math.degrees(value)
                    value = f"math.radians({round(value, 3)})"
                else:
                    value = f"{round(value, 3)}"
            properties[attr] = {
                "default": str(value),
                "type": attr_type,
            }
